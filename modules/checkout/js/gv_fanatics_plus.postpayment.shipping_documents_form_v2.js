	...
	Drupal.behaviors.document_upload_behaviour = {
		attach: function(context, settings) {
			...
			if (jQuery(context).find('.ajax-new-content').length > 0) { // Stop loader
				...
				jQuery('fieldset.documents-wrapper span.file a').each(function() {
			...
			`;
					...
					// Evitar duplicado de iconos y eventos
					if (containerEditDocument[i].hasChildNodes()) {
						for (let j = 0; j < containerEditDocument[i].childNodes.length ; j++) {
							if (containerEditDocument[i].childNodes[j].classList && containerEditDocument[i].childNodes[j].classList.contains('shipping-documents-uploadIcon')) {
								hasIcon = true;
							}
						}
					}
				...
				if (context.tagName == 'DIV' && jQuery(context).children().is('[id*="ajax-wrapper"]')) {
					let newItemURL = jQuery(context).find('a').attr('href');
					let newItemName = jQuery(context).find('a').text();
					let newItemDeleteButton = jQuery(context).find('[id*="file-upload-remove-button"]');
					if (newItemURL && newItemName) {
						let newItemContent = `
							<span class="new-item-title">${newItemName}</span>
							<span class="new-item-anchor"><a href="${newItemURL}" target="_blank">${Drupal.t("DOCUMENTS.PREVIEW")}</a></span>
						`;
						jQuery(context).find('[id*="edit-shipping-documents-"].form-managed-file span').html(newItemContent);

						newItemDeleteButton.appendTo(jQuery(context).find('[id*="edit-shipping-documents-"].form-managed-file span.new-item-title'));
					}
				}
