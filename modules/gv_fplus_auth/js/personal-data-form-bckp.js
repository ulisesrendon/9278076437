...
console.log('testingwebcam', new window.fanaticsPlusWebcam().start);

function createNewWebcam() {
	...
	$('.form-item-image').append(webcamElems);
...
function start() {
	if (!jQuery('.image-preview').attr('src')) {
		var dataSrc = jQuery('.image-preview').attr('data-src');
		if (dataSrc) {
			jQuery('.image-preview').attr('src', dataSrc);
		}	
	}
	
	var webcamWidget = createNewWebcam();
	...
	$('#finish-crop').click(function() {
		...
		$('.image-preview').attr('src', base64Image);
